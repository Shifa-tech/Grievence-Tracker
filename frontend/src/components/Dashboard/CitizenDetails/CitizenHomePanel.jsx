// frontend/src/components/Dashboard/CitizenDetails/CitizenHomePanel.jsx
...
const CitizenHomePanel = ({ user , onGoToSubmit, onGoToTrack  }) => {
  ...
  const fetchUserStats = async () => {
    try {
      ...
      const response = await fetch(`/api/complaint/user/${user?.id}/stats`, {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 font-black uppercase tracking-widest" style={{ color: '#8B4513' }}>Loading dashboard...</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen p-12" style={{ background: '#F5F5DC' }}>
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-2xl border-2 border-[#8B4513] mb-16">
        ...
        <div className="relative px-14 py-20 text-center">
          ...
          <h1 className="text-5xl md:text-6xl font-black uppercase mb-6"
            ...
            Welcome, <span style={{ color: '#FFD700' }}>{user?.username || 'Citizen'}</span>
          </h1>
          <p className="text-lg font-semibold uppercase tracking-widest mb-10" style={{ color: 'rgba(255,215,0,0.7)' }}>
            Your voice matters – help us keep the circus running smoothly
          </p>
      ...
      <div className="mt-16">
        <div className="flex items-center gap-4 mb-12">
          ...
          <h2 className="font-black uppercase tracking-widest text-2xl px-4"
            style={{ fontFamily: 'Georgia, serif', color: '#2F1B0A' }}>✦ Your Complaint Stats ✦</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="rounded-2xl overflow-hidden shadow-lg border-2 border-[#8B4513]">
            <div className="px-6 py-4 text-center" style={{ background: 'linear-gradient(135deg, #2F1B0A, #8B4513)' }}>
              ...
              <h3 className="text-white font-black uppercase tracking-wider mt-2">Total</h3>
            </div>
            <div className="px-6 py-8 text-center" style={{ background: '#F5F5DC' }}>
              <p className="text-5xl font-black text-[#8B4513]">{stats.total}</p>
              <p className="text-sm font-bold uppercase tracking-widest mt-2" style={{ color: '#5a3a1a' }}>Complaints Filed</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden shadow-lg border-2 border-[#8B4513]">
            <div className="px-6 py-4 text-center" style={{ background: 'linear-gradient(135deg, #2F1B0A, #8B4513)' }}>
              <span className="text-3xl">🟡</span>
              <h3 className="text-white font-black uppercase tracking-wider mt-2">Open</h3>
            </div>
            <div className="px-6 py-8 text-center" style={{ background: '#F5F5DC' }}>
              <p className="text-5xl font-black text-[#8B4513]">{stats.open}</p>
              <p className="text-sm font-bold uppercase tracking-widest mt-2" style={{ color: '#5a3a1a' }}>Pending Review</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden shadow-lg border-2 border-[#8B4513]">
            ...
            <div className="px-6 py-8 text-center" style={{ background: '#F5F5DC' }}>
              ...
              <p className="text-sm font-bold uppercase tracking-widest mt-2" style={{ color: '#5a3a1a' }}>Being Fixed</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden shadow-lg border-2 border-[#8B4513]">
            ...
            <div className="px-6 py-8 text-center" style={{ background: '#F5F5DC' }}>
              <p className="text-5xl font-black text-[#8B4513]">{stats.resolved}</p>
              <p className="text-sm font-bold uppercase tracking-widest mt-2" style={{ color: '#5a3a1a' }}>Completed</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-16 rounded-3xl overflow-hidden shadow-2xl border-2 border-[#8B4513]">
        <div className="relative px-10 py-6 overflow-hidden" style={{ background: 'linear-gradient(135deg, #2F1B0A, #8B4513, #2F1B0A)' }}>
          <h2 className="relative font-black uppercase tracking-widest text-xl text-center text-white">⚡ Quick Actions</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-2 gap-6 p-10" style={{ background: '#F5F5DC' }}>
          <button
            onClick={onGoToSubmit}
            className="flex flex-col items-center gap-4 py-8 rounded-2xl border-2 border-[#FFD700] font-black uppercase tracking-wider transition-all duration-200 hover:-translate-y-1 hover:shadow-2xl"
...
export default CitizenHomePanel