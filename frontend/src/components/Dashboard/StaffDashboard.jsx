...
import StaffHomePanel from './StaffDetails/StaffHomePanel'
import StaffAssignments from './StaffDetails/StaffAssignments'
import StaffAllComplaints from './StaffDetails/StaffAllComplaints'

const NAV_ITEMS = [
  { tab: 'dashboard',   icon: '🎪', label: 'Dashboard'       },
  { tab: 'assignments', icon: '👤', label: 'My Assignments'  },
  { tab: 'complaints',  icon: '📋', label: 'All Complaints'  },
]
    ...
    <div className="flex h-screen w-full overflow-hidden" style={{ background: '#F5F5DC' }}>
          ...
          {sidebarOpen && (
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs whitespace-nowrap" style={{ color: 'rgba(255,215,0,0.5)' }}>✦ Ground Crew ✦</p>
              </div>
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex-shrink-0 flex items-center justify-between px-8 py-4 shadow-md"
          ...
          <div>
            <h1 className="text-2xl" style={{ color: 'rgba(255,215,0,0.5)' }}>✦ Ground Crew Dashboard ✦</h1>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-8">
          ...
          {activeTab === 'dashboard' && <StaffHomePanel user={user} />}
          {activeTab === 'assignments' && <StaffAssignments user={user} />}
          {activeTab === 'complaints' && <StaffAllComplaints user={user} />}
